...
public class QRActivity extends Activity implements View.OnClickListener, SensorEventListener{
    ...
    private              String          cropTempPath      = Environment.getExternalStorageDirectory()
                                                                        .getAbsolutePath() + File.separator + "cropQr.jpg";
    ...
    private void initParm (){
        ...
        time_out_time = options.getTime_out_time();
        if (time_out_time != 0){
            startCountDown(time_out_time);
        }
                    ...
                    runOnUiThread(new Runnable(){
                        ...
                        public void run (){
                            ...
                            if (!TextUtils.isEmpty(qrcontent)){
                                ...
                                //尝试用zxing再试一次识别二维码
                                final String qrcontent = QRUtils.getInstance()
                                                                .decodeQRcodeByZxing(imagePath);
                                if (!TextUtils.isEmpty(qrcontent)){
                                    closeProgressDialog();
                                    scanResult.setContent(qrcontent);
                                    scanResult.setType(ScanResult.CODE_QR);
                                    ...
                                    //再试试是不是条形码
                                    try{
                                        String barcontent = QRUtils.getInstance()
                                                                   .decodeBarcode(imagePath);
                                        if (!TextUtils.isEmpty(barcontent)){
                                            ...
                                            QrManager.getInstance()
                                                     ...
                                                     .onScanSuccess(scanResult);
                                            QRUtils.getInstance()
                                                   .deleteTempFile(cropTempPath);//删除裁切的临时文件
                                            finish();
                                        }else{
                                        ...
                                        Toast.makeText(getApplicationContext(), "识别异常！", Toast.LENGTH_SHORT)
                                             .show();
                                        closeProgressDialog();
                                        e.printStackTrace();
                                    }
